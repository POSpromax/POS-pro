...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (!supabaseUrl || !serverKey) return json({ error: 'Server akuntansi belum dikonfigurasi' }, 503);
  ...
  const result = await handleAccountingRequest(request.method, payload, accessToken, admin);
