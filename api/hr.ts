...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (!supabaseUrl || !serverKey) return json({ error: 'Server HR belum dikonfigurasi' }, 503);
  ...
  const result = await handleHrRequest(request.method, payload, accessToken, admin);
