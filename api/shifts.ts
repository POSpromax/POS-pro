...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (!supabaseUrl || !serverKey) return json({ error: 'Server shift belum dikonfigurasi' }, 503);
  ...
  const result = await handleShiftRequest(request.method, payload, accessToken, admin);
