...
## 1.1 COMPLIANCE PEDOMAN MOCKUP REFERENSI (Pointsell & Salesify)

Setiap langkah implementasi teknis UI WAJIB mengacu pada rincian visual mockup (*Pointsell POS UI Kit* & *Salesify Dashboard*):
1. **Product Grid & Cart Layout**:
   - `POSMenuItemCard`: Aspect ratio gambar konsisten, judul tebal, harga oranye (`var(--primary)`), rating bintang (`★ 5.0`), dan button `+`.
   - `CartPanel`: Header ringkas `Order #XXXX`, stepper kuantitas (`- 1 +`), rincian biaya, dan tombol checkout utama **BAYAR / SIMPAN** berlatar oranye solid (`.ui-button-primary`).
2. **Master Data & Inventory Table**:
   - `ui-table`: Header abu-abu terang, status pill `In Stock` / `Lunas` (`.ui-badge-success`), ID produk, serta tombol Edit (hijau) & Delete (oranye/merah).
3. **Analytics Dashboard**:
   - Hero Card Omset: `.ui-card-feature` (oranye gradient background) dengan persentase pertumbuhan (`▲ 9.97% vs last month`).
   - Stat Cards: `.ui-stat-label` (11px uppercase) dan `.ui-stat-value` (28px bold).
   - Filter Periode: Tab pill segmented (`Today | This Week | This Month | This Year`).
