-- ============================================================================
-- Nomor antrean harian.
--
-- order_number ("POS-MSNWCM6VFDL") dibuat dari timestamp base36 supaya unik,
-- tapi 15 karakter acak tidak bisa dibaca kasir maupun pelanggan. Tambahkan
-- nomor urut harian per cabang untuk ditampilkan; order_number tetap ada
-- sebagai identitas unik di struk dan sistem.
--
-- Reset harian memakai zona Asia/Jakarta, bukan UTC — kalau memakai UTC nomor
-- akan berganti pukul 07.00 WIB saat resto sudah melayani pembeli.
-- ============================================================================
...
create index if not exists orders_daily_number_idx
  on public.orders (branch_id, daily_number);

-- Beri nomor pada order yang sudah ada, diurutkan per cabang per hari.
with numbered as (
  select
    id,
    row_number() over (
      partition by branch_id, (created_at at time zone 'Asia/Jakarta')::date
...
begin
  ...
  if v_order_id is null then
    -- Dua kasir menekan Simpan bersamaan tidak boleh dapat nomor antrean sama.
    -- Lock terlepas otomatis saat transaksi selesai.
    v_today := (now() at time zone 'Asia/Jakarta')::date;
    perform pg_advisory_xact_lock(hashtext(v_branch_id::text || '|' || v_today::text));
    ...
    insert into public.orders (
      ...
      order_number, daily_number, source, order_type, status, payment_status, customer_name,
  ...
  -- Kunci pembayaran adalah order_id, bukan id kiriman client: satu order tetap
  -- satu baris payments walau bill diedit dan dibayar ulang berkali-kali.
