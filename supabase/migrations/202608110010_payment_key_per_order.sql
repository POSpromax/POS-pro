-- ============================================================================
-- Perbaikan: satu order hanya boleh punya satu baris pembayaran.
--
-- Di 0009 idempotency_key pembayaran diturunkan dari id yang dikirim client.
-- Nilai itu berubah begitu order tersimpan (client mengirim orders.id, bukan
-- lagi client_request_id), sehingga bill yang diedit lalu dibayar ulang
-- menghasilkan DUA baris payments untuk satu order — omzet terhitung dobel.
--
-- Kuncinya sekarang order_id itu sendiri, ditentukan di dalam function setelah
-- order diketahui. Pembayaran berikutnya atas order yang sama memperbarui
...
-- Rapikan duplikat yang sudah terlanjur dibuat bug 0009: sisakan pembayaran
-- terbaru per order, karena itu yang mencerminkan total bill terakhir.
delete from public.payments p
where exists (
  select 1 from public.payments newer
  where newer.order_id = p.order_id
    and (newer.paid_at, newer.id) > (p.paid_at, p.id)
);
