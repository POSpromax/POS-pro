...
### 2.1.1 ANALISIS DETAIL MOCKUP REFERENSI (Pointsell POS UI Kit & Salesify Dashboard)

Berdasarkan sampel visual mockup yang dilampirkan (*Pointsell POS UI Kit* & *Salesify Dashboard*):

1. **Warna Dominan & Nuansa Brand**:
   - **Canvas Background**: Warm off-white neutral (`#F7F7F5` / `#FAFAF8`) yang memberikan kontras halus dan nyaman dipandang dalam durasi shift panjang.
   - **Card Surface**: Putih bersih (`#FFFFFF`) dengan border halus (`#ECECE8` / `--panel-border`).
   - **Primary Accent**: Vibrant Warm Orange (`#EA580C` / `#FF5B22`) untuk tombol utama, harga produk, badge aktif, dan indikator progres.
   - **Text Hierarchy**: Dark Charcoal (`#20201E`) untuk judul utama, Slate Gray (`#6F706B`) untuk deskripsi/subtext.

2. **Hierarki Tipografi & Kepadatan Karakter (Density)**:
...
3. **Komponen Kartu Produk (Product Card)**:
   - Aspect ratio gambar proporsional dengan corner radius `12px` - `16px`.
   - Judul produk tebal, rating bintang (`★ 5.0`), harga oranye tebal, dan tombol aksi `+ Add Product` / `+`.

4. **Cart Sidebar (`Order #XXXX`)**:
   - Header ringkas dengan nomor order/meja dan tombol `✕`.
   - List item dengan thumbnail gambar + nama item + harga + stepper kuantitas (`- 1 +`).
   - Rincian biaya: Subtotal, Pajak, Diskon, dan Total.
...
5. **Tabel Data (Product / Inventory Table)**:
   - Header abu-abu terang, status badge pill hijau (`In Stock` / `Lunas`), ID produk, serta ikon aksi Edit (hijau) & Delete (oranye/merah).

6. **Dashboard Analytics (Salesify Style)**:
   - **Hero KPI Card**: Card oranye gradient (`--primary-gradient`) untuk metric *Total Sales* dengan teks putih dan badge persentase pertumbuhan (`▲ 9.97% vs last month`).
   - **KPI Metric Cards**: Card putih bersih dengan label upper kecil di atas dan nilai stat besar (28px bold) di bawah.
   - **Time Range Selector Tabs**: Tab pill segmented (`Today | This Week | This Month | This Year`).
