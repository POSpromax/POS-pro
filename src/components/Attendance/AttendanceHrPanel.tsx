...
export function AttendanceHrPanel({ activeUser, staffAccounts, currentBranch, attendanceRecords, terminalMode, onShowToast, initialTab = 'HISTORY' }: Props) {
  ...
  return (
    <section className="mt-6 rounded-2xl border border-[#E4E2DF] bg-white p-4 shadow-sm md:p-6">
      ...
      {tab === 'LEAVE' && <div className="grid gap-5 lg:grid-cols-[minmax(280px,420px)_1fr]">
        ...
        <div className="space-y-2"><h3 className="mb-3 text-sm font-bold">{canManage ? 'Antrean Izin Staff' : 'Riwayat Pengajuan Saya'}</h3>{visibleLeaves.length === 0 ? <p className="rounded-2xl bg-[#F5F5F4] p-8 text-center text-xs font-bold text-slate-400">Belum ada pengajuan izin.</p> : visibleLeaves.map((item) => <div key={item.id} className="rounded-2xl border border-[var(--panel-border)] p-4"><div className="flex flex-wrap justify-between gap-2"><div><p className="text-sm font-bold">{item.staffName}</p><p className="text-[11px] font-bold text-slate-500">{item.leave_type} - {item.start_date} s/d {item.end_date}</p></div><span className={`h-fit rounded-full px-2.5 py-1 text-[10px] font-bold ${item.status === 'APPROVED' ? 'bg-emerald-100 text-emerald-700' : item.status === 'REJECTED' ? 'bg-rose-100 text-rose-700' : 'bg-[var(--brand-100)] text-[var(--primary-text)]'}`}>{item.status}</span></div><p className="mt-2 text-xs text-slate-600">{item.reason}</p>{item.attachment_url && <a href={item.attachment_url} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-[var(--primary-hover)]"><FileCheck2 className="h-3 w-3"/>Lihat lampiran</a>}{canManage && item.status === 'PENDING' && <div className="mt-3 flex gap-2"><button onClick={() => void decideLeave(item.id, 'APPROVED')} className="rounded-lg bg-[var(--primary)] px-3 py-2 text-[11px] font-bold text-white">SETUJUI</button><button onClick={() => void decideLeave(item.id, 'REJECTED')} className="rounded-lg border border-rose-200 px-3 py-2 text-[11px] font-bold text-rose-600">TOLAK</button></div>}</div>)}</div>
      </div>}

      {tab === 'PAYROLL' && canManage && <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-left text-xs"><thead className="border-b text-[11px] font-bold uppercase text-slate-400"><tr><th className="p-3">Staff</th><th>Gaji pokok</th><th>Tunjangan</th><th>Lembur / jam</th><th>Potongan telat / menit</th><th></th></tr></thead><tbody>{branchStaff.map((staff) => { const item = data.payrollProfiles.find((profile) => profile.user_id === staff.id); return <tr key={staff.id} className="border-b border-[var(--panel-border)]"><td className="p-3"><p className="font-bold">{staff.name}</p><p className="text-[10px] font-bold text-slate-400">{staff.role}</p></td><td className="font-bold">{money(item?.base_salary || 0)}</td><td>{money((item?.meal_allowance || 0) + (item?.transport_allowance || 0))}</td><td>{money(item?.overtime_hourly_rate || 0)}</td><td>{money(item?.late_deduction_per_minute || 0)}</td><td><button onClick={() => openPayroll(staff)} className="rounded-lg bg-[var(--brand-50)] px-3 py-2 font-bold text-[var(--primary-hover)]">ATUR</button></td></tr>; })}</tbody></table></div>}

      {detailStaff && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-600/30 p-4"><div className="max-h-[85vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-5"><div className="mb-4 flex items-center justify-between"><div className="flex items-center gap-2"><UserRoundSearch className="h-5 w-5 text-[var(--primary-hover)]"/><div><h3 className="font-bold">{detailStaff.name}</h3><p className="text-[11px] font-bold text-slate-400">RIWAYAT 30 HARI - {detailStaff.role}</p></div></div><button onClick={() => setDetailStaff(null)}><X/></button></div>{detailRecords.length === 0 ? <p className="py-10 text-center text-xs font-bold text-slate-400">Belum ada catatan.</p> : detailRecords.map((record) => <div key={record.id} className="mb-2 flex justify-between rounded-xl border border-[var(--panel-border)] p-3"><div><p className="text-xs font-bold">{record.type.replace('_', ' ')}</p><p className="text-[11px] text-slate-500">{new Date(record.timestamp).toLocaleString('id-ID')}</p></div><span className={`text-[11px] font-bold ${record.status === 'LATE' ? 'text-rose-600' : 'text-emerald-600'}`}>{record.status === 'LATE' ? `TERLAMBAT ${record.minutesLate || 0} MENIT` : 'TEPAT WAKTU'}</span></div>)}</div></div>}

      {payrollStaff && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-600/30 p-4"><div className="w-full max-w-lg rounded-2xl bg-white p-5"><div className="mb-4 flex justify-between"><div className="flex gap-2"><WalletCards className="text-[var(--primary-hover)]"/><div><h3 className="font-bold">Payroll {payrollStaff.name}</h3><p className="text-[11px] font-bold text-slate-400">Nominal bulanan kecuali lembur dan potongan</p></div></div><button onClick={() => setPayrollStaff(null)}><X/></button></div><div className="grid gap-3 sm:grid-cols-2">{Object.entries({ baseSalary: 'Gaji pokok', mealAllowance: 'Uang makan', transportAllowance: 'Transport', overtimeHourlyRate: 'Lembur / jam', lateDeductionPerMinute: 'Potongan telat / menit' }).map(([key,label]) => <label key={key} className="text-[11px] font-bold text-slate-500">{label.toUpperCase()}<input type="number" min="0" value={payroll[key as keyof typeof payroll]} onChange={(e) => setPayroll({ ...payroll, [key]: Number(e.target.value) })} className="mt-1 w-full rounded-xl border border-[var(--panel-border)] p-3 text-sm font-bold"/></label>)}</div><button onClick={() => void savePayroll()} disabled={loading} className="mt-5 w-full rounded-xl bg-[var(--primary)] p-3 text-xs font-bold text-white">SIMPAN KOMPONEN PAYROLL</button></div></div>}
    </section>
  );
}
