    ...
    <div className="flex-1 bg-[#F8FAFC] p-4 md:p-6 overflow-y-auto font-sans select-none flex flex-col justify-between text-slate-900">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
        <div className="flex items-center gap-3">
          ...
          <div className="bg-slate-100 border border-slate-200/80 p-1 rounded-full flex items-center gap-1 shadow-2xs ml-4">
            <button
            ...
            >
              <Package className="w-3.5 h-3.5" /> BAHAN
            </button>

            <button
              onClick={() => setSubTab('DAPUR')}
              className={`px-4 py-1.5 rounded-full text-xs font-black transition-all cursor-pointer flex items-center gap-1.5 ${
                subTab === 'DAPUR'
                  ? 'bg-slate-900 text-white shadow-xs'
                  : 'text-slate-600 hover:text-slate-900 font-bold'
            ...
            >
              <Utensils className="w-3.5 h-3.5" /> DAPUR
            </button>
        ...
        {/* Right Search, Export, and Add Controls */}
        <div className="flex items-center gap-2">
          ...
          {subTab === 'MENU' ? (
            <button
              ...
              className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-full text-xs font-black shadow-xs flex items-center gap-1.5 cursor-pointer active:scale-95 transition-all"
            >
          ...
          <button
            ...
            className="p-2 bg-white border border-slate-200 text-slate-600 hover:bg-slate-100 rounded-full cursor-pointer transition-colors"
      ...
      {/* Top 4 Grayscale & Black/Orange Accent Metric Cards Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {/* MENU */}
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex justify-between items-center">
          <div>
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">MENU</p>
            <p className="text-3xl font-black text-slate-900 mt-1">{menuItems.length}</p>
          </div>
          <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white shadow-xs">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex justify-between items-center">
          <div>
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">ASET</p>
            <p className="text-3xl font-black text-slate-900 mt-1">{totalAssetsCount}</p>
          </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex justify-between items-center">
          <div>
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">RESTOCK</p>
            <p className="text-3xl font-black text-orange-600 mt-1">{restockNeedCount}</p>
          </div>
          <div className="w-12 h-12 bg-orange-600 rounded-2xl flex items-center justify-center text-white shadow-xs shadow-orange-500/20">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs flex justify-between items-center">
          <div>
            <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">KATEGORI</p>
            <p className="text-3xl font-black text-slate-900 mt-1">{categoriesList.length}</p>
          </div>
          <div className="w-12 h-12 bg-slate-100 border border-slate-200 rounded-2xl flex items-center justify-center text-slate-700">
      ...
      {/* Sub-Tab 1: BAHAN & Sub-Tab 2: DAPUR Grid View matching Image 1 & Image 2 */}
      {(subTab === 'BAHAN' || subTab === 'DAPUR') && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3.5">
          {filteredRawList.length === 0 ? (
            <div className="col-span-full bg-white rounded-2xl p-10 text-center text-slate-400 font-bold border border-slate-200">
            ...
            filteredRawList.map((raw) => {
              ...
              return (
                <div
                  key={raw.id}
                  className="bg-white rounded-2xl p-3.5 border border-slate-200/90 shadow-2xs flex flex-col justify-between relative hover:shadow-md transition-shadow"
                >
                  {/* Top Header & Low Stock Badge matching Image 2 */}
                  <div className="flex items-start justify-between gap-1 mb-1">
                    <div className="flex-1 min-w-0">
                      <h3 className="font-black text-xs text-[#1A1714] truncate">{raw.name}</h3>
                    ...
                    {isLow && (
                      <span className="bg-rose-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded-md shadow-xs animate-pulse flex items-center gap-0.5">
                        LOW <span className="font-mono text-[10px]">{raw.stockQuantity}</span>
                      </span>
                    )}
                  </div>

                  {/* Quantity Counter in Big Bold Text matching Image 1 & 2 */}
                  <div className="my-2 text-right">
                    <span className="text-xl font-black text-[#1A1714] tracking-tight">
                      {raw.stockQuantity.toLocaleString('id-ID')}
                    </span>
                  </div>

                  {/* Card Bottom Controls matching Image 1 & 2 */}
                  <div className="flex items-center justify-between border-t border-slate-100 pt-2">
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => handleAdjustStock(raw, 0)}
                        className="p-1 text-sky-500 hover:bg-sky-50 rounded-lg cursor-pointer"
                        title="Sync Stock"
                      >
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="w-6 h-6 rounded-full bg-rose-50 border border-rose-200 text-rose-600 hover:bg-rose-100 flex items-center justify-center cursor-pointer transition-colors"
                      >
                        <Minus className="w-3 h-3" />
                      </button>
                      <button
                        onClick={() => handleAdjustStock(raw, 1)}
                        className="w-6 h-6 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-600 hover:bg-emerald-100 flex items-center justify-center cursor-pointer transition-colors"
                      >
      ...
      {/* Sub-Tab 3: MENU List View matching Image 3 */}
      {subTab === 'MENU' && (
        <div className="space-y-4">
          {/* Category Bar Creator matching Image 3 */}
          <div className="flex items-center gap-3">
            <input
              type="text"
              ...
              className="bg-white border border-[#E5E5E7] rounded-full px-4 py-2 text-xs font-bold text-slate-800 outline-none focus:border-indigo-500 w-64 shadow-2xs"
            />
            <button
              onClick={handleAddCategory}
              className="px-5 py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-full text-xs font-black shadow-xs flex items-center gap-1 cursor-pointer transition-all active:scale-95"
            >
              <Plus className="w-4 h-4" /> Tambah
            </button>
          </div>

          {/* Categories Accordion Group matching Image 3 */}
          {categoriesList.map((cat) => {
            ...
            return (
              <div key={cat} className="bg-white rounded-2xl border border-slate-200/90 shadow-2xs overflow-hidden">
                {/* Category Group Header matching Image 3 */}
                <div
                  onClick={() =>
                    setExpandedCategories((prev) => ({ ...prev, [cat]: !prev[cat] }))
                  }
                  className="p-4 bg-[#FAFAF8] flex items-center justify-between border-b border-slate-100 cursor-pointer hover:bg-slate-100/60 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    {isExpanded ? <ChevronUp className="w-4 h-4 text-slate-400" /> : <ChevronDown className="w-4 h-4 text-slate-400" />}
                    <h3 className="font-black text-sm text-[#1A1714] uppercase">{cat}</h3>
                ...
                {/* Category Items List matching Image 3 */}
                {isExpanded && (
                  <div className="divide-y divide-slate-100">
                    {categoryItems.map((item) => (
                      <div
                        key={item.id}
                        className="p-3.5 flex items-center justify-between hover:bg-[#FAFAF8] transition-colors"
                      >
                        <div className="flex items-center gap-3">
                          <img
                            src={item.image}
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-black text-xs text-[#1A1714]">{item.name}</span>
                              {item.isAutoStock !== false && (
                                <span className="bg-emerald-50 text-emerald-600 border border-emerald-200 text-[9px] font-black px-1.5 py-0.5 rounded">
                                  Auto-Stock
                                </span>
                              )}
                            </div>
                        ...
                        <div className="flex items-center gap-6">
                          <span className="font-black text-xs text-[#1A1714]">
                            Rp {item.price.toLocaleString('id-ID')}
                          </span>

                          <span className="bg-slate-100 text-slate-700 text-xs font-black px-3 py-1 rounded-md border border-slate-200">
                            {item.stockCount || 100} Porsi
                          </span>
      ...
      {/* Sub-Tab 4: LAPORAN Inventory Summary */}
      {subTab === 'LAPORAN' && (
        <div className="bg-white rounded-2xl p-6 border border-slate-200/90 shadow-2xs space-y-4 font-sans">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-4 bg-[#FAFAF8] rounded-2xl border border-slate-200">
              <p className="text-xs font-bold text-slate-400">Total Nilai Bahan Baku (Aset):</p>
            ...
            <div className="p-4 bg-[#FAFAF8] rounded-2xl border border-slate-200">
              <p className="text-xs font-bold text-slate-400">Rata-rata Margin Keuntungan HPP:</p>
            ...
            <div className="p-4 bg-[#FAFAF8] rounded-2xl border border-slate-200">
              <p className="text-xs font-bold text-slate-400">Bahan Baku Perlu Restock:</p>
              <p className="text-xl font-black text-rose-600 mt-1">{restockNeedCount} Item</p>
            </div>
          </div>
        </div>
      )}

      {/* EDIT MENU & RESEP / KOMPOSISI Modal matching Image 4 EXACTLY */}
      {isEditMenuModalOpen && editingMenu && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <form
            ...
            className="bg-white w-full max-w-3xl rounded-3xl p-6 shadow-2xl space-y-5 font-sans text-slate-900 border border-slate-200 max-h-[90vh] overflow-y-auto"
          >
            {/* Modal Header matching Image 4 */}
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h2 className="text-base font-black text-[#1A1714] tracking-tight uppercase">EDIT MENU</h2>
              <button
                type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* LEFT COLUMN: INFO PRODUK matching Image 4 */}
              <div className="space-y-4">
                ...
                <div className="flex gap-3 items-start">
                  <div className="relative group shrink-0">
                    ...
                    />
                    <label className="absolute inset-0 bg-slate-950/60 rounded-2xl flex flex-col items-center justify-center text-white text-[9px] font-black opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                      ...
                      <input
                        ...
                        accept="image/*"
                        className="hidden"
                        onChange={handleUploadMenuPhoto}
                      />
                    </label>
                  </div>

                  <div className="flex-1 space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-wider block">FOTO & NAMA MENU</label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-2.5 text-xs font-black text-slate-900 outline-none focus:border-slate-900 focus:bg-white"
                    />

                    <label className="inline-flex items-center gap-1.5 px-3 py-1 bg-orange-50 border border-orange-200 text-orange-700 rounded-xl text-[10px] font-bold cursor-pointer hover:bg-orange-100/80 transition-colors">
                      <Upload className="w-3.5 h-3.5" />
                      <span>{isUploadingPhoto ? 'Mengunggah ke Cloudinary...' : 'Upload Foto Lokal (Cloudinary)'}</span>
                      <input
                        type="file"
                        accept="image/*"
                        className="hidden"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-2.5 text-xs font-bold text-slate-900 outline-none focus:border-slate-900 focus:bg-white"
                    />
                  </div>
                </div>

                {/* Checkbox Gunakan Resep (Auto-Stock) matching Image 4 */}
                <div className="bg-slate-50 border border-slate-200 p-3 rounded-2xl flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="auto-stock-chk" className="text-xs font-black text-slate-900 cursor-pointer uppercase">
                    GUNAKAN RESEP (AUTO-STOCK)
                  </label>
                </div>
              </div>

              {/* RIGHT COLUMN: RESEP & KOMPOSISI matching Image 4 */}
              <div className="space-y-4">
                <label className="text-xs font-black text-orange-600 uppercase tracking-wider block border-b border-orange-100 pb-1">
                  RESEP & KOMPOSISI
                </label>

                {/* Recipe Select Input Bar matching Image 4 */}
                <div className="flex items-center gap-2">
                  <select
                    value={selectedRecipeMaterialId}
                    onChange={(e) => setSelectedRecipeMaterialId(e.target.value)}
                    className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl p-2.5 text-xs font-bold outline-none focus:border-slate-900 focus:bg-white text-slate-900"
                  >
                  ...
                  <input
                    ...
                    className="w-16 bg-slate-50 border border-slate-200 rounded-2xl p-2.5 text-xs font-black text-center outline-none focus:border-slate-900 focus:bg-white text-slate-900"
                  />

                  <button
                    type="button"
                    onClick={handleAddIngredientToRecipe}
                    className="w-9 h-9 bg-orange-600 hover:bg-orange-700 text-white rounded-2xl flex items-center justify-center font-black cursor-pointer shadow-xs"
                  >
                    <Plus className="w-5 h-5" />
                  </button>
                </div>

                {/* Recipe Ingredients Box matching Image 4 */}
                <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 space-y-2 min-h-48 max-h-56 overflow-y-auto">
                  {editingMenu.ingredients && editingMenu.ingredients.length > 0 ? (
                    editingMenu.ingredients.map((ing) => (
                      ...
                      >
                        <span className="font-black text-xs text-slate-900">{ing.rawMaterialName}</span>
                        <div className="flex items-center gap-3">
                          <span className="font-black text-xs text-slate-600 font-mono">
                    ...
                    <div className="h-40 flex items-center justify-center text-xs font-bold text-slate-400 italic">
            ...
            {/* Bottom Save Action Button matching Image 4 */}
            <div className="pt-2">
              <button
                type="submit"
                className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 active:scale-95 text-white font-black text-xs rounded-2xl shadow-md uppercase tracking-wider transition-all cursor-pointer"
              >
      ...
      {isRawModalOpen && editingRaw && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <form
            ...
            className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl space-y-4 font-sans text-slate-900 border border-slate-200"
          >
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <div className="pt-2">
              <button
                type="submit"
                className="w-full py-3.5 bg-slate-900 hover:bg-slate-800 text-white font-black text-xs rounded-2xl shadow-md uppercase tracking-wider transition-all cursor-pointer"
              >
                Simpan Bahan Baku
              </button>
            </div>
